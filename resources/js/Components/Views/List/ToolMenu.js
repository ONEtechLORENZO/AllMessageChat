...
export default function ToolMenu(props) {
    ...
    return(
        <Menu as="div" className="inline-block text-left ml-auto">
            <div>
                <Menu.Button                                               
                    className="w-12 h-12 bg-white rounded shadow-sm flex justify-center items-center "
                >
                    <BsTools />                                              
                </Menu.Button>
            </div>
            ...
            >
                <Menu.Items className={"absolute whitespace-nowrap right-9 mt-2 w-auto min-w-[500px] max-w-xl  z-[500] p-6  origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none overflow-y-auto"} >
                    <div className='font-semibold text-xl'>{props.translator['Customize view']}</div>
                    <div className='flex mt-2'>
                        {props.module == 'Contact' && (customHeader).map((header, key) => {
                            return(
                                <div className='flex px-2 ml-2'>
                                    <div className="flex items-center h-5">                                
                                        <input 
                                            ...
                                            checked={columnOptions[header.name]}
                                        />
                                    </div>
                    ...
                    <div className='!mt-4 grid grid-cols-3 gap-7'>
                        {fieldGroup &&
                          Object.entries(fieldGroup).map(([group, fields]) => (
                            <div>
                              <div className='text-sm font-semibold'>{props.translator[group]}</div>
                               <ul role="list" className="m-h-64 !mt-1 !pl-0 pr-2">
                                    {(fields).map((field) => {
                                        if(field.name == 'emails' || field.name == 'phones' || field.name == 'images') {
                                            return;
                                        }
                                        return(
                                                    ...
                                                    <div className="flex items-start">
                                                        <div className="flex items-center h-5">                                
                                                            <input 
                                                                ...
                                                                checked={columnOptions[field.name]}
                                                            />
                                                        </div>
                                                        <div className="ml-3 text-sm">
                                                            <label htmlFor="terms_condition" title="Click here to read it" className="font-medium text-gray-700" >

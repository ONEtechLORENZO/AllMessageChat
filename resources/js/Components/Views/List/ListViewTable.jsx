                    ...
                    <thead >
                        <tr className='text-[#363740] text-sm'>
                            ...
                            {Object.entries(headers).map(([name, field]) => {
                                let sort_order = 'desc';
                                let sortable = true;
                                if ((props.sort_by && props.sort_by == name) || (props.sort_by == 'last_name' && name == 'widget_name')) {
                                    if (props.sort_order == 'desc') {
                                        sort_order = 'asc';
                                    }
                                }
                                if (name == 'widget_tag' || name == 'widget_contacts' || name == 'widget_socials' || name == 'widget_list') {
                                    sortable = false;
                                }

                                return (
                                    <th scope="col" key={name} className="py-3.5 pl-4 pr-3 text-left text-sm font-medium text-gray-900 sm:pl-4">
                                        <span className="pb-[6px] border-b border-[#C8C8C8] block text-gray-900" onClick={() => { sortable ? sortColumn(name, sort_order) : '' }}>
                                            {field.label}
                                        </span>
                                    </th>
                                )
                        ...
                        {Object.entries(props.records).map(([key, record]) => {
                            ...
                            return (
                                <tr key={key} className="bg-white">
                                    ...
                                    {Object.entries(headers).map(([name, field], index) => {
                                        let title = '';
                                        let widget = false;
                                        let tmpWidgets = [];
                                        let column_value = record[name] ? record[name] : '';
                                        ...
                                        if (name == 'widget_name') {
                                            let first_name = record['first_name'] ? record['first_name'] : ' ';
                                            let last_name = record['last_name'] ? record['last_name'] : ' ';
                                        ...
                                        if (name == 'widget_socials') {
                                            widget = true;
                                            tmpWidgets.push(<div className='inline-grid gap-2 grid-cols-4 text-[#7666B4]'>
                                                {record['facebook_username'] && <BsFacebook className='text-indigo-600' />}
                                                {record['whatsapp_number'] && <BsWhatsapp className='text-green-500' />}
                                                {record['instagram_username'] && <BsInstagram className='text-pink-500' />}
                                                {record['telegram_number'] && <BsTelegram className='text-blue-400' />}
                                                {record['tiktok_username'] && <FaTiktok className='text-gray-600' />}
                                                {record['linkedin_username'] && <BsLinkedin className='text-blue-600' />}
                                            </div>);
                                        }
                                            ...
                                            if (record.tags) {
                                                ...
                                                })
                                                {
                                                    (record.tags).length > 2 ?
                                                        tmpWidgets.push(<div className='flex items-center text-[#7666B4]'> + {((record['tags']).length - 2)} </div>)
                                                        : ''
                                                }
                                            }
                                        }
                                        ...
                                        if (name == 'widget_list') {
                                            if (record.categorys) {
                                                widget = true;
                                                (record.categorys).map((category, categoryIndex) => {
                                                    if (categoryIndex === 0 || categoryIndex === 1) {
                                                        tmpWidgets.push(<span className="inline-flex items-center !rounded bg-[#f6d41b61] px-2 text-xs font-semibold leading-5 text-[#bd810e]">
                                                            {category.name}
                                                        </span>)
                                                    }
                                                })
                                                {
                                                    (record.categorys).length > 2 ?
                                                        tmpWidgets.push(<div className='flex items-center text-[#7666B4]'> + {((record['categorys']).length - 2)} </div>)
                                                        : ''
                                                }
                                            }
                                        }
                                            ...
                                            column_value =
                                                <div className="ml-4">
                                                    <div className="font-medium text-gray-900 ">
                                                        <Link href={route(url, { id: record.id })} className='cursor-pointer underline'>
                                                            {column_value}
                                                        </Link>
                                                    </div>
                                                    {record.updated_at &&
                                                        <div className="text-gray-500 flex items-center gap-1"><WiTime8 /> <ListViewTimeFormate time={record.updated_at} /></div>
                                                    }
                                                </div>
                                        }
                                        ...
                                        if (field.name == 'url' && column_value) {
                                            column_value = <a href={column_value} target='_blank'>{column_value.substring(0, 40)}</a>;
                                        }
                                    ...
                                    <td>
                                        {props.actions && props.module != 'Message' ?
                                            <ActionMenu
                                                record={record}
                                                fields={fields}
                                                {...props}

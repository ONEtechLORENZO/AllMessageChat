...
import { PencilSquareIcon, TrashIcon, EllipsisVerticalIcon, EyeIcon, LinkIcon, ArrowDownTrayIcon } from '@heroicons/react/24/solid';
                ...
                <Menu.Items className="absolute z-20 right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div className="px-1 py-1 ">
                                ...
                                {({ active }) => (
                                    <>
                                        {props.edit_link ?
                                            ...
                                            >
                                                <PencilSquareIcon className='h-4 w-4 cursor-pointer text-indigo-700' /> {props.translator['Edit']}
                                            </Link>
                                            :
                                            <button className='flex gap-2 p-2 items-center text-gray-700' onClick={() => props.showEditForm(props.record.id)}>
                                                <PencilSquareIcon className='w-4 h-4 cursor-pointer text-indigo-700' /> {props.translator['Edit']}
                                            </button>
                                        }
                                    </>
                                )}
                        ...
                        {props.actions.download === true && props.module === 'Document' ?
                            <Menu.Item>
                                {({ active }) => (
                                    <a href={route('download_document', props.record.id)} className="flex px-2 text-gray-700"><ArrowDownTrayIcon className='h-4 w-4 cursor-pointer m-1 text-indigo-700' /> Download</a>
                                )}
                            </Menu.Item>
                            : ''}
                        {props.actions.download === true && props.record.status === 'success' ?
                            <Menu.Item>
                                {({ active }) => (
                                    <a href={route('invoices', props.record.id)} className="flex px-2 text-gray-700" ><ArrowDownTrayIcon className='h-4 w-4 cursor-pointer text-indigo-700' /> Download</a>
                                )}
                            </Menu.Item>
                            : ''}

...
class HandleImport extends Command
{
        ...
        if($import){
            ...
            foreach ($csvRecords as $index =>  $csvRecord) {
                ...
                if($module_name == 'Contact') {  
                    ...
                    unset($records[$index]['company_id']);
                }
                ...
                if($module_name == 'Contact') {
                    foreach($fields as $field) {
                        if($field->field_type == 'phones') {
                            $phoneNumbers = $records[$index][$field->field_name];
                            if($phoneNumbers){
                                $phones[$index] = explode(', ', $phoneNumbers);
                            }
                            unset($records[$index][$field->field_name]);
                        }
                        if($field->field_type == 'emails') {
                            $Emails = $records[$index][$field->field_name];
                            if($Emails){
                                $emails[$index] = explode(', ', $Emails);
                            }
                            unset($records[$index][$field->field_name]);
                        }
                    }
                ...
                if($module_name == 'Order') {
                    $lineItems[] = $this->createLineItems($order_id, $mapping_items, $csvRecord, $lineItems, $current_date);
                }
            }
            ...
            if($module_name == 'Contact') {
                $this->importContactOneByOne($records, $phones, $emails);
            } else {
    ...
    public function createLineItems($order_id, $fields, $records, $items, $current_date) {
        ...
        if($product_name){
            $product = Product::where('name', $product_name)->first(); // Check the product details
    ...
    public function importContactOneByOne($records, $phones, $emails) {
        ...
        foreach($records as $index => $record) {
            ...
            if(isset($phones[$index]) && !empty($phones[$index])) {
                foreach($phones[$index] as $number) {
                    $phone = new Phone();
                    $phone->phones = $number;
            ...
            if(isset($emails[$index]) && !empty($emails[$index])) {
                foreach($emails[$index] as $mailAddress) {
                    $email = new Email();
                    $email->emails = $mailAddress;
